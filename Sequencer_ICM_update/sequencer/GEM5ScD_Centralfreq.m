...
P.frequency     = Trans.frequency; % in MHz

% For PI/SHI
%--- Tx strategy for PI (number of steering will be half of P.nbangles )
P.nbAngles      = 32;  % Steering angles for PW in Tx [deg]
P.frameRate     = 100; % 450; % Frame rate [Hz]
P.nTXperAngle   = 1;   % 2 Transmit for Pulse Inversion/ 3 pulse inversion +shi
...
%%
P.Sampling      = 'BS100BW'; %'NS200BW';% sampling mode : 'BS100BW' 'BS50BW'
P.Voltage       = 40;        % Voltage
...
P.simu      = 0;
P.ECGrec    = 0;
P.PauseBetweenNbFrames = 1;

%% Display
P.Dyn           = 40;      % Dynamic range Display
P.Saturation    = 10;      % Saturation Display
P.NframeDisplay = 2;       % P.nbAcq;       % NbFrameForDisplay
...
%% Pulse Inversion
RawFondamental  = double( bufferRealigned);
...
%     IqRawFondamental = filtfiltGPU(B, A, IqRawFondamental)*2; % factor 2: to preserve the envelope amplitude
    IqRawFondamental = single(gather(IqRawFondamental));
elseif strcmp(Receive(1).sampleMode, 'BS100BW')
